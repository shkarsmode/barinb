...
import { createPersonFromNewPerson } from 'src/store/person.util';
import { PersonStorage } from 'src/storage/person.storage';
  ...
  removePersonBuilding$ = createEffect(() =>
    this.actions$.pipe(
      ...
      fetch({
        ...
        run: (action, personsEntities) => {
          const person = personsEntities ? personsEntities[action.payload.id] : null;
          const buildings = person?.buildings.filter((building:any) => building !== action.payload.building) ?? [];
